...
- `codex-observe paths --json` verifies the local path handoff without scanning private logs and puts the human-readable `private-validate --newest-files 25 --serve` review first, keeps the guided `private-validate --newest-files 25 --json` command discoverable for automation, and then lists lower-level manual follow-ups, while `private-validate --visual --json` remains the one-command private browser-evidence path after dependencies are installed.
