...
- [ ] `codex-observe evidence-bundle --out .artifacts/public-evidence` creates a reviewer-facing synthetic bundle with a reviewer README containing key findings, terminal key findings, a review checklist and reproduce-local commands, `LIMITATIONS.md`, report, comparison, audit, visual QA artifacts, and `codex-observe.evidence-bundle.v1` manifest evidence. This bundle is optional/reviewer-facing and still local-only by default.
