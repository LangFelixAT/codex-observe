...
- [ ] `codex-observe sessions --db .artifacts/demo/codex_observe_demo.sqlite --json` lists aggregate-only session summaries with a plain-text recommended-action block, `schema_version`, `status`, a structured `recommended_session`, `recommendation_detail`, and structured `next_commands` for the highest-risk run, and missing database JSON remains machine-readable.
- [ ] `codex-observe report --db .artifacts/demo/codex_observe_demo.sqlite --out .artifacts/demo/run-report.md` creates an aggregate-only Markdown report with a top-level recommended action, cost-profile evidence, aggregate triage assessment, and a next-run success target and follow-up command templates.
...
- [ ] `codex-observe evidence-bundle --out .artifacts/public-evidence` creates a reviewer-facing synthetic bundle with terminal output and a reviewer README containing key findings, a review checklist and reproduce-local commands, `LIMITATIONS.md`, `PUBLIC_TOUR_FEEDBACK.md`, report, comparison, audit, visual QA artifacts, and `codex-observe.evidence-bundle.v1` manifest evidence. This bundle is optional/reviewer-facing and still local-only by default.
