...
- [ ] `codex-observe sessions --db .artifacts/demo/codex_observe_demo.sqlite --json` lists aggregate-only session summaries with a plain-text Tool out column and recommended-action block with largest-tool-output driver evidence, `schema_version`, `status`, a structured `recommended_session`, aggregate-driver `recommendation_detail`, and structured `next_commands` for the highest-risk run, and missing database JSON remains machine-readable.
