...
- [ ] `codex-observe evidence-bundle --out .artifacts/public-evidence` creates a reviewer-facing synthetic bundle with a terminal output and reviewer README containing key findings, a review checklist and reproduce-local commands, `LIMITATIONS.md`, `PUBLIC_TOUR_FEEDBACK.md`, report, comparison, audit, visual QA artifacts, and `codex-observe.evidence-bundle.v1` manifest evidence. This bundle is optional/reviewer-facing and still local-only by default.
