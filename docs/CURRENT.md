...
The current core loop is printed by `codex-observe tour` with evidence and success checks; `codex-observe tour --json` exposes the same synthetic-data commands and per-step success checks as a schema-versioned automation contract. The loop is:
...
2. Inspect the Overview operator briefing, triage, comparison quick-read and metric delta cards, report and comparison download controls, diagnostics, cost-share metric cards, Agent detail thread brief, Timeline quick read, Tools quick read, Duplication quick read, and Raw tables data inventory.
...
7. Run `codex-observe evidence-bundle --out .artifacts/public-evidence` when reviewers need one synthetic local directory with terminal output and a reviewer README containing key findings, a review checklist, reproduce-local commands, next-run validation commands, `LIMITATIONS.md`, `PUBLIC_TOUR_FEEDBACK.md`, report, comparison, audit, visual QA artifacts, and the `codex-observe.evidence-bundle.v1` manifest.
