...
7. Run `codex-observe evidence-bundle --out .artifacts/public-evidence` when reviewers need one synthetic local directory with a reviewer README containing key findings, terminal key findings, a review checklist and reproduce-local commands, `LIMITATIONS.md`, report, comparison, audit, visual QA artifacts, and the `codex-observe.evidence-bundle.v1` manifest.
