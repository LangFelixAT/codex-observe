...
7. Run `codex-observe evidence-bundle --out .artifacts/public-evidence` when reviewers need one synthetic local directory with terminal output and a reviewer README containing an ordered action plan, key findings, a review checklist with comparison review-path guidance, reproduce-local commands, next-run validation commands, `LIMITATIONS.md`, `PUBLIC_TOUR_FEEDBACK.md`, report, comparison, audit, visual QA artifacts, and the `codex-observe.evidence-bundle.v1` manifest.
