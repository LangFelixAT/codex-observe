...
The current core loop is printed by `codex-observe tour` with a top-level review checklist, evidence, success checks, and a final terminal `Next commands` footer; `codex-observe tour --json` exposes the same synthetic-data commands, including plain terminal handoff commands, `review_path`, and per-step success checks as a schema-versioned automation contract. The loop is:
