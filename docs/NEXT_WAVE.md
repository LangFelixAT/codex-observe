...
- [x] The manifest records local URL, database source, viewport sizes, screenshot filenames, clicked tabs, Agent detail selector exercise, sidebar risk labels, and expected high-risk default metric cards.
- [x] The script checks minimum screenshot dimensions and obvious blank/exception states.
- [x] README and PR template ask contributors to attach or reference the manifest for UI-facing changes, and `codex-observe audit` verifies saved manifest schema/contract evidence, referenced screenshots, layout review, sidebar-risk labels, and high-risk metric evidence after visual QA runs.
