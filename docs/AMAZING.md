...
- The start and resume journeys now put the product answer first: help and README expose synthetic-demo and real-session-review paths, the tour reaches diagnosis/habit/proof before optional reviewer material, paths prefer the human review over JSON automation, and sessions explain the selected run and any same-band tie-break before the table. The dashboard briefing precedes metrics and is proven fully visible in initial desktop and narrow viewports.
