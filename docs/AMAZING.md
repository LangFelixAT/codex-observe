...
- The README now frames the product value, source-install policy, multi-session synthetic demo path, privacy-safe commands, supported log shapes, visual QA evidence with missing/empty database onboarding states, sidebar risk labels, sidebar snapshot-count evidence, and expected high-risk metrics, aggregate comparison workflow, contributor workflow, and links to `docs/LIMITATIONS.md` for known limitations and next-work sources.
