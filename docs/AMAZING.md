...
- The dashboard now has a stronger first-run experience with copy-pasteable empty-state commands, first-class `codex-observe demo` and privacy-safe `codex-observe doctor` commands with copy-pasteable recovery hints, a two-session synthetic demo with contrasting high- and low-risk runs, a risk-aware dashboard default and sidebar risk labels, a tab-covering local and CI visual QA workflow with screenshot quality checks plus structured quick-read manifest evidence for missing/empty database onboarding states, sidebar risk labels, high-risk metric cards, Overview comparison quick-read and metric delta cards plus report/comparison download controls, and the Overview operator briefing, aggregate report artifact export, dashboard comparison quick-read and metric delta cards plus Markdown/JSON report and comparison downloads, share-aware Overview cost metrics, a polished actionable diagnostics card summary, Agent detail thread brief, Timeline quick read, Tools quick read, Duplication quick read, Raw tables data inventory, a ranked opportunity stack plus a next-run playbook that turn diagnostics into impact-targeted workflow habits, a privacy-safe session listing command, a privacy-safe report export with a quick-read headline, triage assessment, and measurable next-run success target and follow-up command templates for sharing run evidence, and aggregate comparisons that show opportunity-change movement and preserve diagnostic priority in next-step recommendations. Release/privacy checks live in `docs/RELEASE.md`, and the current source-install distribution policy lives in `docs/DISTRIBUTION.md`.
...
- The workflow generates synthetic demo data via `codex-observe demo`, clicks all dashboard tabs, exercises the Agent detail selector, captures desktop and narrow screenshots, and records missing/empty database onboarding states, sidebar risk labels, expected high-risk default metric cards, comparison quick-read and metric delta cards plus report/comparison download controls, and operator-briefing evidence in the manifest.
