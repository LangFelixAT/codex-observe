...
- The start and resume journeys now put the product answer first: help and README expose synthetic-demo and real-session-review paths, the concise tour stops after diagnosis/habit/proof and points to explicit `--reviewer` mode, paths prefer the human review over JSON automation, and sessions explain the selected run and any same-band tie-break before the table. The dashboard briefing and all six tabs are proven visible in initial desktop and narrow viewports; Overview puts the checklist and native copy-ready next-run prompt before supporting metrics.
- Repeated use now starts from a relevant comparison: the dashboard defaults to the immediate chronological follow-up, or nearest predecessor, labels the relationship, and keeps manual selection available; synthetic and ignored real-profile visual QA verify the selected relationship at desktop and narrow widths.
