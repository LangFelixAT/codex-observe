...
- Issues #10-#19 closed after dashboard guidance alignment, answer-first start/resume work, the concise-tour split, action-first dashboard hierarchy, chronological comparison correction, adjacent-run comparison defaults, comparison-first proof flow, composable Focus filtering, bounded 50-conversation history rendering, and schema-v2 visual-evidence integrity passed local, synthetic and real-profile visual, clean-install, and CI gates. Issue #19 isolated test fixtures from saved screenshots, added exact byte-size and SHA-256 verification, and synchronized every manifest-referenced onboarding capture into release-audit evidence. Completed local draft files were retired after publication or completion instead of being republished as stale issues. Future issue drafts should represent fresh work only; public-tour observations now have a privacy-safe feedback template before they become implementation issues; release-candidate UX evidence and public README tour polish are implemented locally, the completed `009` draft is implemented locally by `codex-observe evidence-bundle` with a reviewer README, bundled limitations doc, and retired from publishable issue state, and real-log parser and dashboard feedback checkpoints have been exercised through the approved local sessions path while future fixture promotion remains HITL-only and privacy-reviewed.
- Issue #20 is the active finalization pass: make the existing product presentable through reliable CI, a concise product-first README, reviewed synthetic visuals, curated release notes, and a verified GitHub v0.3.0 release. No dashboard, parser, report, or CLI features are in scope.
