...
This ingests the newest 25 files by default, checks database health, lists aggregate sessions, exports the recommended aggregate report under `.artifacts/private/`, prints a copy-pasteable `python scripts/visual_qa.py --profile real` command for ignored private dashboard evidence, and, with `--serve`, opens the dashboard on that private database. Terminal output stays free of private counts, filenames, raw IDs, prompts, tool output, and raw log content.
