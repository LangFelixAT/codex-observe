...
This ingests the newest 25 files by default, checks database health, lists aggregate sessions, exports the recommended aggregate report under `.artifacts/private/`, and, with `--serve`, opens the dashboard on that private database. Terminal output stays free of private counts, filenames, raw IDs, prompts, tool output, and raw log content.
