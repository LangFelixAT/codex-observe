...
A new user can evaluate the product without private logs. `codex-observe tour` prints this path from the terminal with a top-level review checklist, evidence and success checks to look for at each step, and a final copy-pasteable `Next commands` footer; `codex-observe tour --json` emits the same synthetic-data path plus plain terminal handoff commands, `codex-observe.tour.v1` schema metadata, `review_path`, `feedback_handoff`, and per-step success checks for automation:
