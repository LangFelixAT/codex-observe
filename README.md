...
```
Then open <http://127.0.0.1:8501>. The sidebar keeps the default all-runs review path, its Find session box can narrow large histories by session fragment, preview label, date, or risk, and its Risk filter can focus on one triage band without losing the full Overview context.
...
The command prints the resolved local Codex sessions path, the Codex Observe database path, existence checks, and next commands without scanning logs or printing filenames, prompts, tool output, session IDs, or aggregate metrics. Its first next command is the guided `codex-observe private-validate ... --json` loop, followed by lower-level ingest/doctor/session commands. Use that resolved path for private validation when you want real Codex history rather than synthetic demo input.

To run the bounded private validation loop into ignored artifacts before opening the dashboard; use the printed `--all` follow-up when you are ready to validate the full local history:

```bash
codex-observe private-validate ~/.codex/sessions --visual --serve --host 127.0.0.1 --port 8501
```

This ingests the newest 25 files by default, checks database health, lists aggregate sessions, exports the recommended aggregate report under `.artifacts/private/`, can run real-profile browser visual QA with `--visual` into ignored private dashboard evidence, still prints the copy-pasteable `python scripts/visual_qa.py --profile real` command, and, with `--serve`, opens the dashboard on that private database. Terminal output stays free of private counts, filenames, raw IDs, prompts, tool output, and raw log content.
...
4. Run `codex-observe sessions --db .artifacts/demo/codex_observe_demo.sqlite` to list reportable runs with aggregate triage risk, a recommended-action block, aggregate Focus, Duration, Snapshots, Tool out, and Guardian columns, a review path, and terminal `Next commands`; output is capped at 50 sessions by default, with `--limit <n>` for larger reviews and `--risk high|medium|low|unknown` when you want to narrow a large history to one triage band. Run `codex-observe sessions --db .artifacts/demo/codex_observe_demo.sqlite --json` for `schema_version`, `status`, `total_sessions`, `returned_sessions`, `truncated`, aggregate `risk_distribution`, per-session `usage_snapshots`, `session_duration_hours`, `focus_driver`, `focus_label`, a structured `recommended_session`, `recommendation_detail` with raw aggregate driver fields, session-duration metadata, and ordered `driver_summary` display labels, a structured `review_path` for report, next-run validation, compare, and safe feedback steps, and structured `next_commands` for baseline report export, next-run report export, and comparison automation without printing prompts or tool output; the demo includes a newer low-risk follow-up so the highest-risk recommendation is visible.
