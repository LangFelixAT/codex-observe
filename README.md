...
4. Run `codex-observe sessions --db .artifacts/demo/codex_observe_demo.sqlite` to list reportable runs with aggregate triage risk, a recommended-action block, an aggregate Tool out column, a review path, and terminal `Next commands`; run `codex-observe sessions --db .artifacts/demo/codex_observe_demo.sqlite --json` for `schema_version`, `status`, a structured `recommended_session`, `recommendation_detail` with raw aggregate driver fields and ordered `driver_summary` display labels, a structured `review_path` for report, compare, next-run validation, and safe feedback steps, and structured `next_commands` without printing prompts or tool output; the demo includes a newer low-risk follow-up so the highest-risk recommendation is visible.
