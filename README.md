...
A new user can evaluate the product without private logs. `codex-observe tour` prints this path from the terminal with a top-level review checklist plus evidence and success checks to look for at each step, and `codex-observe tour --json` emits the same synthetic-data path plus plain terminal handoff commands, `codex-observe.tour.v1` schema metadata, `review_path`, and per-step success checks for automation:
...
5. Run `codex-observe report --db .artifacts/demo/codex_observe_demo.sqlite --out .artifacts/demo/run-report.md`, then `codex-observe report --db .artifacts/demo/codex_observe_demo.sqlite --format json --out .artifacts/demo/run-report.json`, to export aggregate-only evidence with a quick-read headline, top-level recommended action, terminal success-target and next-command confirmation, ranked opportunity stack, recommended next habit, next-run success target, follow-up command templates, and structured next-action target; the dashboard Overview exposes the same selected-run Markdown/JSON report downloads and aggregate comparison quick-read, review-path, metric delta, and next validation command cards plus Markdown/JSON downloads for reviewers who start in the UI.
6. Run `codex-observe compare --before-report .artifacts/demo/run-report.json --after-report .artifacts/demo/run-report.json --out .artifacts/demo/run-comparison.md`, then `codex-observe compare --before-report .artifacts/demo/run-report.json --after-report .artifacts/demo/run-report.json --format json --out .artifacts/demo/run-comparison.json`, to compare workflow changes with a verdict, largest-change summary, top-level recommended action, terminal next validation command, terminal next-command guidance, opportunity-change summary, percent-delta table, diagnostic-change summary, structured recommendation target, structured review path, follow-up command templates, and triage-risk movement without exposing raw content.
