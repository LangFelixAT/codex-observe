...
2. Inspect the initial-viewport operator briefing and complete tab navigation, then use the Overview checklist and native copy-ready next-run prompt before supporting metrics. Compare runs uses explicit chronological Before/After context; the synthetic older high-risk run to newer low-risk follow-up reports improvement. Continue through portfolio and risk context, review path, triage, downloads, comparison evidence, diagnostics, and the Agent detail, Timeline & jumps, Tools, Duplication, and Raw tables views.
