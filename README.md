...
`codex-observe tour` remains available as the optional reviewer and audit workflow. Lower-level `ingest`, `doctor`, `sessions`, `report`, `compare`, and `scan-and-serve` commands remain available for manual control.

## Public Tour

A new user can evaluate the product without private logs. `codex-observe tour` starts with the diagnosis, recommended habit, and proof target, then presents the optional reviewer workflow with evidence and success checks, a terminal feedback handoff, and a final copy-pasteable `Next commands` footer; `codex-observe tour --json` emits the same synthetic-data path plus plain terminal handoff commands, `codex-observe.tour.v1` schema metadata, `review_path`, `feedback_handoff`, and per-step success checks for automation:
