...
- Desktop screenshot:
- Narrow screenshot:
- Visual QA manifest: schema-v2, path-safe `.artifacts/visual/visual-qa-manifest.json` with exact byte-size and SHA-256 integrity evidence, referenced screenshot files, layout review, sidebar risk labels, expected high-risk metric card evidence, comparison review-path and usage-snapshot metric delta evidence, report-and-comparison-download-control evidence, and operator-briefing evidence
...
- Tabs exercised: Overview, Agent detail, Timeline quick read, Tools quick read, Duplication quick read, Raw tables data inventory
