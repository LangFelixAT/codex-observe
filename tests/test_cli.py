...
def write_valid_visual_manifest(root: Path) -> None:
    ...
    for name, viewport in viewports.items():
        ...
        viewport_results[name] = {
            ...
            "sidebar_history_page": dict(cli.EXPECTED_VISUAL_SIDEBAR_HISTORY_PAGE),
...
def test_visual_manifest_evidence_failures_validate_saved_sidebar_metric_and_success_target_evidence(
    ...
    payload["viewports"]["desktop"].pop("sidebar_history_page")
    ...
    assert (
        "visual QA manifest missing desktop sidebar history page evidence" in failures
    )
    assert (
...
def test_public_tour_payload_is_private_log_free_and_points_to_visual_verification() -> (
    ...
    assert any("report sampled-ingest warning" in item for item in evidence)
    assert any("comparison sampled-ingest warning" in item for item in evidence)
